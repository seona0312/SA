...
void main()
{
	...
	printf ("\n\nCiphertext(%d) : ", nOutputTextLen);
	for (i=0;i<nOutputTextLen;i++)	{printf("%02X ",OutputBytes[i]);}
	...
	else
	{
		...
		for (i = 0; i < message_length - process_blockLeng; )
		{
			memcpy( pbszOutputText, OutputBytes + i, process_blockLeng );
			data = chartoint32_for_HIGHT_CTR(pbszOutputText, process_blockLeng);
			HIGHT_CTR_Process( &info, data, process_blockLeng, outbuf, &nOutLeng );
			cdata = int32tochar_for_HIGHT_CTR( outbuf, nOutLeng);
			memcpy( InputBytes + i, cdata, nOutLeng );
			i += nOutLeng;
		}
		remainleng = message_length % process_blockLeng;
		if (remainleng == 0)
		...
		memcpy( pbszOutputText, OutputBytes + i, remainleng );
		data = chartoint32_for_HIGHT_CTR(pbszOutputText, remainleng);
		HIGHT_CTR_Process( &info, data, remainleng, outbuf, &nOutLeng );
		HIGHT_CTR_Close( &info, outbuf, nOutLeng, &nEncRmainLeng );
		cdata = int32tochar_for_HIGHT_CTR(outbuf, nOutLeng - nEncRmainLeng);
		memcpy(InputBytes + i, cdata, nOutLeng - nEncRmainLeng);
	...
	printf ("\n\nPlaintext(%d)  : ", nOutputTextLen);
	for (i=0;i<nOutputTextLen;i++)	{printf("%02X ",InputBytes[i]);}
