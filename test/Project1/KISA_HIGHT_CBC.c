...
void KISA_HIGHT_CBC_main()
{
	...
	printf ("Ciphertext(%d) : ", nCipherTextLen);
	...
	printf ("Plaintext(%d)  : ", nPlainTextLen);
...
#define INIT_VECTOR_SIZE 8

typedef struct lsa_hight_key_st
{
	int key_data[128];
} LSA_HIGHT_KEY;

typedef struct lsa_hight_info_st
{
	int								encrypt;
	unsigned char			ivec[4];
	LSA_HIGHT_KEY			hight_key;
	unsigned char			userKey[16];
	unsigned char			cbc_buffer[2];
	int								buffer_length;
	unsigned char			cbc_last_block[2];
} LSA_HIGHT_INFO;


int* LSA_chartoint_for_HIGHT_CBC(unsigned char* in, int inLen)
{
	int* data = malloc(sizeof(int) * inLen);
	int len, i;
...
int LSA_HIGHT_CBC_init(KISA_HIGHT_INFO* pInfo, int mode, unsigned char* pUserKey, unsigned char* pbIV)
{
	int i, j;

	if (NULL == pInfo || NULL == pUserKey || NULL == pbIV)
		return 0;

	memset(pInfo, 0, sizeof(KISA_HIGHT_INFO));

	pInfo->encrypt = mode;
	memcpy((unsigned char*)pInfo->ivec, (unsigned char*)pbIV, INIT_VECTOR_SIZE);	//BLOCK_SIZE_HIGHT=8
	memcpy(pInfo->userKey, pUserKey, 16);

	for (i = 0; i < INIT_VECTOR_SIZE; i++)
	{
		for (j = 0; j < INIT_VECTOR_SIZE; j++)
			pInfo->hight_key.key_data[16 * i + j] = pUserKey[(j - i) & 7] + Delta[16 * i + j];
...
void LSA_HIGHT_ECB_encrypt_forCBC(BYTE* pbszIN_Key128, BYTE* pbszUserKey, const unsigned char* in, unsigned char* out)
{
	register unsigned char t0, t1, t2, t3, t4, t5, t6, t7;
	BYTE* key, * key2;
...
int LSA_HIGHT_CBC_Process(KISA_HIGHT_INFO* pInfo, unsigned char* in, int inLen, unsigned char* out, int* outLen)
{
	//다중 블럭 암호화, 복호화 함수
	int nCurrentCount = INIT_VECTOR_SIZE;
	unsigned char* pdwXOR;
	...
	if (KISA_ENCRYPT == pInfo->encrypt)
	{
		...
		while (nCurrentCount <= inLen)
		{
			...
			KISA_HIGHT_ECB_encrypt_forCBC(pInfo->hight_key.key_data, pInfo->userKey, (BYTE*)out, (BYTE*)out);
			pdwXOR = out;
			nCurrentCount += INIT_VECTOR_SIZE;
			in += BLOCK_SIZE_HIGHT_INT;
			out += BLOCK_SIZE_HIGHT_INT;
		}

		*outLen = nCurrentCount - INIT_VECTOR_SIZE;
		pInfo->buffer_length = inLen - *outLen;

		memcpy(pInfo->ivec, pdwXOR, INIT_VECTOR_SIZE);
		memcpy(pInfo->cbc_buffer, in, pInfo->buffer_length);
	...
	else
	{
		...
		*outLen = nCurrentCount - INIT_VECTOR_SIZE;
		memcpy(pInfo->ivec, pdwXOR, INIT_VECTOR_SIZE);
		memcpy(pInfo->cbc_last_block, out - BLOCK_SIZE_HIGHT_INT, INIT_VECTOR_SIZE);
	}

	return 1;
}

int LSA_HIGHT_CBC_Encrypt(unsigned char* pbszUserKey, unsigned char* pbszIV, unsigned char* pbszPlainText, int in_offset, int nPlainTextLen, int* pbszCipherText)
{
	KISA_HIGHT_INFO info;
	unsigned char* outbuf = { 0 };
	unsigned char* data = { 0 };
	unsigned char* newpbszPlainText = { 0 };
	...
	LSA_HIGHT_CBC_init(&info, KISA_ENCRYPT, pbszUserKey, pbszIV);

	outlen = ((nPlainTextLen + nPlainTextPadding)/8) * 4;
	outbuf = malloc(sizeof(unsigned int) * outlen);
	
	data = LSA_chartoint_for_HIGHT_CBC(newpbszPlainText, nPlainTextLen);

	LSA_HIGHT_CBC_Process(&info, data, nPlainTextLen, outbuf, &nRetOutLeng);
	HIGHT_CBC_Close(&info, outbuf, nRetOutLeng, &nPaddingLeng);
	cdata = int32tochar_for_HIGHT_CBC(outbuf, nRetOutLeng + nPaddingLeng);
	memcpy(pbszCipherText, cdata, nRetOutLeng + nPaddingLeng);
	free(data);
...
void LSA_HIGHT_CBC_main()
{
	unsigned char pbszUserKey[16] = { 0x88, 0xe3, 0x4f, 0x8f, 0x08, 0x17, 0x79, 0xf1, 0xe9, 0xf3, 0x94, 0x37, 0x0a, 0xd4, 0x05, 0x89 };
	unsigned char pbszIV[INIT_VECTOR_SIZE] = { 0x26, 0x8d, 0x66, 0xa7, 0x35, 0xa8, 0x1a, 0x81 };	//초기화벡터(난수)
	unsigned char plainText[71] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F,
		0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F,
		0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F,
		0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F,
		0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06 };
	unsigned char pbszCipherText[80] = { 0x00 };
	unsigned char pbszPlainText[80] = { 0x00 };
	...
	printf("Key        : ");
	Display(pbszUserKey, sizeof(pbszUserKey)/sizeof(unsigned char));

	printf("IV         : ");
	Display(pbszIV, sizeof(pbszIV)/sizeof(unsigned char));

	printf("Plaintext(%d)  : ", nPlainTextLen);
	Display(plainText, nPlainTextLen);


	printf("\n\n");
	printf("\nEncryption....\n");
	nCipherTextLen = LSA_HIGHT_CBC_Encrypt(pbszUserKey, pbszIV, plainText, 0, nPlainTextLen, pbszCipherText);
	printf("Plaintext(%d)  : ", nCipherTextLen);
	Display(pbszCipherText, nCipherTextLen);
